        ...
        if(len(sys.argv) > 1):
            if(sys.argv[1] == "--repeat"):
                if ipv4_enabled and ipv6_enabled:
                    print("🕰️  Updating IPv4 (A) & IPv6 (AAAA) records every %r seconds" % (repeat_interval))
                elif ipv4_enabled and not ipv6_enabled:
                    print("🕰️  Updating IPv4 (A) records every %r seconds" % (repeat_interval))
                elif ipv6_enabled and not ipv4_enabled:
                    print("🕰️  Updating IPv6 (AAAA) records every %r seconds" % (repeat_interval))
                next_time = time.time()
                killer = GracefulExit()
                prev_ips = None
                while True:
